...
// デフォルトテクスチャ格納ディレクトリ
std::string SpriteCommon::kDfautTextureDirectoryPath="Resources/";
...
void SpriteCommon::Initialize(DirectXCommon* dxCommon_)
{
	...
	result = dxCommon->GetDevice()->CreateRootSignature(0, rootSigBlob->GetBufferPointer(), rootSigBlob->GetBufferSize(),
		IID_PPV_ARGS(&roorSignature));
	assert(SUCCEEDED(result));
	rootSigBlob->Release();
	// パイプラインにルートシグネチャをセット
	pipelineDesc.pRootSignature = roorSignature.Get();
...
void SpriteCommon::LoadTexture(uint32_t index, const std::string& fileName)
{
	...
	std::string fullPath = kDfautTextureDirectoryPath + fileName;
